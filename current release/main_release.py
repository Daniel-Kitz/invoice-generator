...
from borb.pdf import PDF, Alignment, Document
from borb.pdf import FixedColumnWidthTable as Table
from borb.pdf import FlexibleColumnWidthTable as FlexTable
...
def get_information():
    ...
    while x == "y":
        ...
        if x == "y":
            item = ["Anzahl", "Einheit", "Bezeichnung", "Einzelpreis"]
    ...
    for key, value in infoDict[2].items():
        match key:
            case "date":
                pass
            case "bank":
                table.add(TableCell(Paragraph(value, font_size=Decimal(11))))
                table.add(
                    TableCell(
                        Paragraph(
                            "Steuernummer: " + infoDict[0]["issuerid"],
            ...
            case "blz":
                if value != " ":
                    table.add(
                        TableCell(
                            Paragraph(
                                str(key).upper() + ": " + value, font_size=Decimal(11)
                            )
                        )
                    )
                    table.add(TableCell(Paragraph(" ", font_size=Decimal(11))))
                else:
                    table.add(TableCell(Paragraph(" ", font_size=Decimal(11))))
                    table.add(TableCell(Paragraph(" ", font_size=Decimal(11))))
            case _:
...
def mainitem_table(item):
    ...
    for i in ["Anzahl", "Einheit", "Bezeichnung", "Einzelpreis", "Gesamtpreis"]:
        table.add(TableCell(Paragraph(i, font="Helvetica-Bold")))
    ...
    for i in range(len(item)):
        ...
        for k in range(len(current)):
            match k:
                ...
                case 2:
                    table.add(TableCell(Paragraph(current[k])))
                case 3:
                    value = current[k]
                    ...
                    table.add(
                        TableCell(
                            Paragraph(
                                value_string, horizontal_alignment=Alignment.RIGHT
                            )
                        )
                    )
                case _:
    ...
    table.add(TableCell(Paragraph("Gesamtbetrag: ", font="Helvetica-Bold"), col_span=4))
...
def invoice_info(invoice_date):
    ...
    table.add(Paragraph("Rechnungs-Nr.: ", horizontal_alignment=Alignment.RIGHT)).add(
        Paragraph(" ")
    )
...
def issuer_table_top(issuerDict):
    table = Table(
        number_of_columns=2, number_of_rows=4, column_widths=[Decimal(1), Decimal(2)]
    )

    table.add(Paragraph(" ")).add(
        Paragraph(
            issuerDict["issuername"],
    ...
    table.add(Paragraph(" ")).add(
        Paragraph(
            "{} {}".format(issuerDict["issuerzip"], issuerDict["issuercity"]),
            horizontal_alignment=Alignment.RIGHT,
        )
    )
    table.add(Paragraph(" ")).add(
        Paragraph(
            "Steuernummer: {}".format(issuerDict["issuerid"]),
            horizontal_alignment=Alignment.RIGHT,
        )
    )
    ...
    small_info = Paragraph(
        "{} - {} - {} {}".format(
            issuerDict["issuername"],
            issuerDict["issueraddress"],
            issuerDict["issuerzip"],
            issuerDict["issuercity"],
        ),
        border_bottom=True,
...
def receiver_table_information(clientDict):
    ...
    table.add(
        Paragraph("{} {}".format(clientDict["clientzip"], clientDict["clientcity"]))
    )
...
def additional_information_bottom():
    x = input("add additional information? y/n ")
    if x == "y":
...
def main():
    ...
    layout.add(Paragraph(additional_information_bottom()))
    layout.add(
        Paragraph(
    ...
    filename = (
        mainDict[0][0]["issuername"]
        + "-"
        + mainDict[0][1]["clientname"]
        + "-"
        + mainDict[0][2]["date"]
        + ".pdf"
    )
